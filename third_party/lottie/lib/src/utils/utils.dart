...
class Utils {
  ...
  static Path applyTrimPathContentIfNeeded(
    Path path,
  ...
  /// Returns [path] trimmed to `[startValue, endValue]` (plus [offsetValue]).
  ///
  /// The result is a new [Path] when any trim is applied. Callers must not
  /// `path.set(extractPath(path))`: on Flutter web, `extractPath` lazily
  /// references the source, so assigning it back cycles `LazyPath.builtPath`
  /// and stack-overflows CanvasKit (#411).
  static Path applyTrimPathIfNeeded(
    Path path,
    double startValue,
    double endValue,
    double offsetValue,
    ...
    if (newStart == newEnd) {
      L.endSection('applyTrimPathIfNeeded');
      return Path()..fillType = path.fillType;
    }
