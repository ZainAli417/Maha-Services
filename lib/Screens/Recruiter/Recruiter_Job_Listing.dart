    ...
    return jobs.where((job) {
      ...
      if (_searchQuery.isNotEmpty) {
        ...
        final qualifications = (job['qualifications'] as String? ?? '')
            .toLowerCase();
        final workModes =
            (job['workModes'] as List<dynamic>?)
                ?.cast<String>()
                .join(' ')
                .toLowerCase() ??
            '';
        final searchableText =
            '$title $company $department $location $experience $pay $qualifications $workModes';
      ...
      if (_selectedWorkMode != 'All') {
        final workModes =
            (job['workModes'] as List<dynamic>?)?.cast<String>() ?? [];
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: SizedBox(
                width: 1400,
                child: Column(
                  children: [_buildListHeader(), _buildJobsList(filteredJobs)],
                ),
              ),
            ),
          ),
      ...
      child: filteredJobs.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) {
                return _JobListItem(jobData: filteredJobs[index], index: index);
              },
            ),
    );
  }
...
class _JobListItem extends StatefulWidget {
  ...
  final int index;
  const _JobListItem({required this.jobData, required this.index});
  @override
