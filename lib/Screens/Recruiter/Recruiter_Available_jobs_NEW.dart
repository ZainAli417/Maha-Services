...
class _JobListView_NewState extends State<JobListView_New> {
  ...
  void _applyFilters() {
    ...
    setState(() {
      _filteredJobs = widget.jobs.where((job) {
        ...
        if (_selectedCompany != null && job['company'] != _selectedCompany) return false;
        if (_selectedDepartment != null && job['department'] != _selectedDepartment) return false;
        if (_selectedLocation != null && job['location'] != _selectedLocation) return false;
        if (_selectedJobType != null && job['nature'] != _selectedJobType) return false;
  ...
  void _clearFilters() {
    ...
    setState(() {
      ...
      _selectedCompany = _selectedDepartment = _selectedLocation = _selectedJobType = null;
  ...
  Widget _buildFilterDropdown(String label, String? value, List<String> items, IconData icon, Function(String?) onChanged) {
    ...
    return PopupMenuButton<String>(
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
        decoration: BoxDecoration(
          color: value != null ? _primary.withOpacity(0.1) : _background,
          borderRadius: BorderRadius.circular(10),
        ...
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(icon, size: 16, color: value != null ? _primary : _textSecondary),
            const SizedBox(width: 6),
            Text(
              value ?? label,
              style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w500, color: value != null ? _primary : _textSecondary),
            ),
            const SizedBox(width: 4),
            Icon(Icons.arrow_drop_down, size: 18, color: value != null ? _primary : _textSecondary),
          ],
        ),
      ),
      onSelected: (v) {
      ...
      itemBuilder: (context) => [
        PopupMenuItem(value: null, child: Text('All', style: GoogleFonts.poppins(fontSize: 13))),
        ...items.map((i) => PopupMenuItem(value: i, child: Text(i, style: GoogleFonts.poppins(fontSize: 13)))),
      ],
    );
  }
  ...
  Widget _buildJobList() {
    ...
    return GridView.builder(
      ...
      gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
        ...
        crossAxisSpacing: 24,
        mainAxisSpacing: 24,
        childAspectRatio: 1.5, // Adjust this for card height (1.2 = taller, 1.8 = shorter)
      ),
