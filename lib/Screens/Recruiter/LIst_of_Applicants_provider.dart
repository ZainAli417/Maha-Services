...
class ApplicantRecord {
  ...
  factory ApplicantRecord.fromSnapshot(
    ...
    final firstEdu = educations.isNotEmpty
        ? educations.first
        : <String, dynamic>{};
    final firstExp = experiences.isNotEmpty
        ? experiences.first
        : <String, dynamic>{};
...
class ApplicantsProvider with ChangeNotifier {
      ...
      if (retirementStatusFilter != 'All') {
        ...
        try {
          final diff =
              ...
              365;
          final ok = retirementStatusFilter == 'Within 1 Year'
              ? (diff <= 1 && diff >= 0)
              : retirementStatusFilter == '1-3 Years'
              ? (diff > 1 && diff <= 3)
              : retirementStatusFilter == '3-5 Years'
              ? (diff > 3 && diff <= 5)
              : retirementStatusFilter == '5+ Years'
              ? diff > 5
              : false;
          if (!ok) return false;
        } catch (_) {
  ...
  void _sort() {
    ...
    switch (sortBy) {
      case 'applied_desc':
        _filtered.sort((a, b) => b.appliedAt.compareTo(a.appliedAt));
        break;
      case 'applied_asc':
        _filtered.sort((a, b) => a.appliedAt.compareTo(b.appliedAt));
        break;
      case 'name_asc':
      ...
      case 'experience_desc':
        _filtered.sort(
          (a, b) => b.experienceYears.compareTo(a.experienceYears),
        );
        break;
    }
  }
  ...
  void updateExperienceYearsRange(RangeValues v) =>
      _set(() => experienceYearsRange = v);
